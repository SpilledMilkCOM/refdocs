...
Planned Revisions:
	...
	4) Setup www.PayPal.com account for "time saved" donations -- psmart@SpilledMilk.com  (Give it a test, it might be working now).
	5) Attempt to automatically adjust the environment to switch to text output and change the column width that is larger (if needed).
...
SET @theExec = 'SELECT CONVERT(VARCHAR(' + @dataWidth + '), ''' + @newLine + 'INSERT INTO ' + @tableName + ' ' + @newLine
